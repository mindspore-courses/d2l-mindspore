...
class SGD(nn.Cell):
    ...
    def construct(self, grads):
        for idx in range(len(self.parameters)):
            ops.assign(self.parameters[idx], self.parameters[idx] - self.lr * grads[idx])
        return True
...
def download(name, cache_dir=os.path.join('..', 'data')):
    ...
    r = requests.get(url, stream=True, verify=True, timeout=(10, 10))
...
def train_batch_ch13(train_step, X, y): # , loss, trainer, devices
    """用多GPU进行小批量训练"""
