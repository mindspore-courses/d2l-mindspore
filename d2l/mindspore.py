...
def load_array(data_arrays, batch_size, is_train=True):
    ...
    dataset = GeneratorTensorDataset(dataset=dataset, batch_size=batch_size, shuffle=is_train)
    return dataset


def default_collate(data):
    unit_x = []
    unit_y = []
    for unit in data:
        unit_x.append(unit[0])
        unit_y.append(unit[1])
    return d2l.stack(unit_x, 0), d2l.stack(unit_y, 0)


class GeneratorTensorDataset():
    def __init__(self, dataset, batch_size=1, shuffle=None, sampler=None, batch_sampler=None, collate_fn=None,
                 drop_last=False):
...
class RandomSampler(Sampler):
    def __init__(self, data_source, num_samples=None) -> None:
...
def synthetic_data(w, b, num_examples):
    ...
    X = d2l.normal((num_examples, len(w)), 0, 1)
    # y = np.matmul(X, w) + b
    y = d2l.matmul(X, w) + b
    # y += np.random.normal(0, 0.01, y.shape)
    y += d2l.normal(y.shape, 0, 0.01)
...
class DotProductAttention(nn.Cell):
    ...
    def construct(self, queries, keys, values, valid_lens=None):
        d = queries.shape[-1]
        scores = ops.BatchMatMul()(queries, keys.swapaxes(1, 2)) / ops.Sqrt()(
            ops.ScalarToTensor()(d, mindspore.float32))
